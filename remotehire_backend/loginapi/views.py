    ...
    try:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=True, suffix='.jpg') as tmp:
                ...
                result = inference.predict_image(tmp.name)
                if not result:
                    return Response({'verdict': 'unknown', 'confidence': 0, 'message': 'Model failed to produce a result.'}, status=status.HTTP_200_OK)

                # inference.predict_image returns confidence in [0,1]
        ...
        try:
            ...
            # Determine the target user for verification (the OTHER participant)
            target_user = interview.candidate if interview.recruiter == user else interview.recruiter

            if not getattr(target_user, 'photo', None):
                # No reference photo for comparison
                return Response({'verdict': 'unknown', 'confidence': 0, 'message': 'No reference photo available for comparison.'}, status=status.HTTP_200_OK)

            # Load reference image bytes
            ref_bytes = None
            try:
                if hasattr(target_user.photo, 'path') and target_user.photo.path:
                    with open(target_user.photo.path, 'rb') as f:
                        ref_bytes = f.read()
                else:
                    # Try to fetch via URL if available
                    import requests
                    ref_url = getattr(target_user.photo, 'url', None)
                    if ref_url:
                        r = requests.get(ref_url)
                        if r.status_code == 200:
                            ref_bytes = r.content
            except Exception as e:
                print(f"[DEEPFAKE_CHECK] Error loading reference photo: {e}")

            if not ref_bytes:
                return Response({'verdict': 'unknown', 'confidence': 0, 'message': 'Reference photo could not be loaded.'}, status=status.HTTP_200_OK)

            # Write both images to temp files for face_recognition
            with tempfile.NamedTemporaryFile(delete=True) as tmp_input, tempfile.NamedTemporaryFile(delete=True) as tmp_ref:
                tmp_input.write(img_bytes)
                tmp_input.flush()
                tmp_ref.write(ref_bytes)
                tmp_ref.flush()

                input_img = face_recognition.load_image_file(tmp_input.name)
                ref_img = face_recognition.load_image_file(tmp_ref.name)

                input_enc = face_recognition.face_encodings(input_img)
                ref_enc = face_recognition.face_encodings(ref_img)

                if not input_enc:
                    return Response({'verdict': 'unknown', 'confidence': 0, 'message': 'No face detected in input image.'}, status=status.HTTP_200_OK)
                if not ref_enc:
                    return Response({'verdict': 'unknown', 'confidence': 0, 'message': 'No face found in reference photo.'}, status=status.HTTP_200_OK)

                # Use first encodings
                input_vec = input_enc[0]
                ref_vec = ref_enc[0]
                from numpy import linalg
                import numpy as np
                dist = np.linalg.norm(input_vec - ref_vec)

                # Convert distance to heuristic confidence (0 distance -> 100%)
                # Typical threshold for same person ~0.6; map [0,0.6] -> [100,50], >0.6 decreases
                if dist < 0.4:
                    confidence = 98 - (dist * 10)
                elif dist <= 0.6:
                    confidence = 90 - ((dist - 0.4) * 200)
                else:
                    confidence = max(0, 50 - ((dist - 0.6) * 80))

                confidence = float(max(0, min(100, round(confidence, 2))))
                verdict = 'real' if dist <= 0.6 else 'unknown'

                return Response({'verdict': verdict, 'confidence': confidence, 'distance': float(dist)}, status=status.HTTP_200_OK)

        except ImportError:
            # face_recognition not installed — fall back to OpenCV face detection only
            try:
                import cv2
                import numpy as np
                # Decode image bytes
                ...
                if img_cv is None:
                    return Response({'error': 'Failed to decode image for analysis.'}, status=status.HTTP_400_BAD_REQUEST)
                gray = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)
                face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
                faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5)
                if len(faces) > 0:
                    # Heuristic confidence based on largest face area ratio
                    x, y, w, h = max(faces, key=lambda f: f[2] * f[3])
                    face_area = w * h
                    total_area = img_cv.shape[0] * img_cv.shape[1]
                    ratio = face_area / total_area
                    confidence = float(min(99, max(30, round(ratio * 1000, 2))))
                    return Response({'verdict': 'real', 'confidence': confidence, 'message': 'Face detected (opencv haarcascade).'}, status=status.HTTP_200_OK)
                else:
                    return Response({'verdict': 'unknown', 'confidence': 0, 'message': 'No face detected.'}, status=status.HTTP_200_OK)
            except ImportError:
                return Response({'error': 'Deepfake detection libraries not installed. Install `face_recognition` or `opencv-python` on the server.'}, status=501)
            except Exception as e:
                print(f"[DEEPFAKE_CHECK] OpenCV error: {str(e)}")
                return Response({'error': f'OpenCV processing error: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
            print(f"[DEEPFAKE_CHECK] face_recognition error: {str(e)}")
